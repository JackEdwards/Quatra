    ...
    for (EntityPtr& entity : entities) {
        if (KeyFitsLock(entity->m_types)) {
            ...
            if (p_input->m_fireKeyPressed) {
                ...
                entityQueue.push_back(std::make_unique<Bullet>(position, velocity, rotation));
            }
        }
    }
    
    entities.insert(entities.end(), std::make_move_iterator(entityQueue.begin()), std::make_move_iterator(entityQueue.end()));
}